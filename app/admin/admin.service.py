...
def list_users(db: Session, skip: int = 0, limit: int = 50) -> list[AdminUserItem]:
    users = db.query(User).offset(skip).limit(limit).all()
    return [AdminUserItem.model_validate(u) for u in users]


def update_user(db: Session, user_id: int, data: AdminUserUpdate) -> AdminUserItem | None:
...
def list_skills(db: Session, skip: int = 0, limit: int = 100) -> list[AdminSkillItem]:
...
def create_skill(db: Session, data: AdminSkillCreate) -> AdminSkillItem:
