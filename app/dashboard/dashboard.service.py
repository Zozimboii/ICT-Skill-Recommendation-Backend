...
def _is_meaningful(importance: str, freq: float | None) -> bool:
    """meaningful = required หรือ recommended ที่ผ่าน frequency filter"""
...
def _should_include(freq: float | None, importance: str) -> bool:
    """include ใน missing list (รวม optional ที่ freq สูงด้วย)"""
...
def get_skill_gap(db: Session, user_id: int) -> list[SkillGapResponse]:
    ...
    for rec in recommendations:
        ...
        matched_meaningful_count = sum(
            ...
            if _is_meaningful(_importance_tier(imp), skill.frequency_score)
        )
...
def get_recommendations(db: Session, user_id: int) -> list[RecommendationItem]:
    ...
    for rec in recs:
        ...
        matched_meaningful = sum(
            ...
            if rs.match_type == "matched"
            and _is_meaningful(_importance_tier(imp or 0.5), skill.frequency_score)
        )
