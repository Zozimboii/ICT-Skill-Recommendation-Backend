...
from app.schemas.assessment import PositionItem, PositionSkillItem, PositionSkillsResponse

MIN_JOBS = 3  # ต้องมี job ≥ 3 ถึงจะแสดง position


def get_positions(db: Session) -> list[PositionItem]:
...
def get_position_skills(db: Session, sub_category_id: int) -> PositionSkillsResponse | None:
    """
...
def reset_assessment_skills(db: Session, user_id: int) -> int:
