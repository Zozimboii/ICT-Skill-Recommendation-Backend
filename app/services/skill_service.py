...
class SkillService:
    ...
    def get_or_create_skill(self, db, skill_name: str, skill_type: str, category_id: int = None):
        skill = db.query(Skill).filter(Skill.name.ilike(skill_name)).first()
        
        if skill:
            print(f"    [SKILL] existing: id={skill.id} name='{skill.name}'")
            return skill
    
        print(f"    [SKILL] creating new: name='{skill_name}' type={skill_type} category_id={category_id}")
        new_skill = Skill(
            name=skill_name,
        ...
        print(f"    [SKILL] created: id={new_skill.id}")
        return new_skill